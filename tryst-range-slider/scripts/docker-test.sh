#!/bin/sh
# Builds the tryst-range-slider Docker test image and runs its spec
...
#
#   tryst-range-slider/scripts/docker-test.sh                          # everything
#   tryst-range-slider/scripts/docker-test.sh spec/tryst/range_slider_spec.cr  # one file
set -eu

IMAGE=tryst-range-slider-test
LABEL=project=tryst-range-slider

# This script lives in <repo>/tryst-range-slider/scripts, so the root is
# two up.
ROOT=$(CDPATH= cd -- "$(dirname -- "$0")/../.." && pwd)

docker build --label "$LABEL" -t "$IMAGE" -f "$ROOT/tryst-range-slider/Dockerfile" "$ROOT"
