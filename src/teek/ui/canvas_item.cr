...
module Teek
  module UI
    ...
    class CanvasItem
      ...
      # call needed here. Teek::CanvasBindInterceptor (registered for the
      # "canvas" widget type) already reconciles the callback this
      # registers when the item's tag/id stops matching anything, the
      # same leak-safety Teek::App#bind gives ordinary widget events -
      # nothing extra to do here for that. subs reuses Teek::App::BIND_SUBS
      # for the same symbol -> %-code vocabulary Teek::App#bind already
      # uses.
      private def bind_item_event(event : String, handler : Proc(Array(String), CallbackSignal, Nil), subs : Array(Symbol) = [] of Symbol) : Nil
        ...
        subs.each { |sub| args << Teek::App::BIND_SUBS[sub] }
        @app.command(@canvas_path, args, EMPTY_KWARGS)
      end
    end
  end
