...
{% unless flag?(:windows) %}
  ...
  # our own small event-proc later, the same shape Tryst::EventSource already
  ...
  # is waiting for. A bare `Tryst::App.new; interp.tcl_eval("update")`, no
  ...
  lib LibTcl
    ...
    # - see tryst_notifier_file_handler_event_proc.
  ...
  module Tryst
    ...
    class Notifier
      ...
      # arguments, the same pattern tryst_crystal_callback_dispatch already
      # uses in interp.cr.
      ...
      def self.poll_once(state : ThreadState) : Bool
        ...
        pollfds.each_with_index do |pfd, i|
          ...
          event.value.header.proc = ->tryst_notifier_file_handler_event_proc(Void*, LibC::Int)
          event.value.fd = fd
      ...
      def self.install_once : Nil
        @@install_lock.synchronize do
          ...
          procs = LibTcl::NotifierProcs.new(
            set_timer_proc: ->tryst_notifier_set_timer(LibTcl::Time*),
            wait_for_event_proc: ->tryst_notifier_wait_for_event(LibTcl::Time*),
            create_file_handler_proc: ->tryst_notifier_create_file_handler(LibC::Int, LibC::Int, Void*, Void*),
            delete_file_handler_proc: ->tryst_notifier_delete_file_handler(LibC::Int),
            init_notifier_proc: ->tryst_notifier_init,
            finalize_notifier_proc: ->tryst_notifier_finalize(Void*),
            alert_notifier_proc: ->tryst_notifier_alert(Void*),
            service_mode_hook_proc: ->tryst_notifier_service_mode_hook(LibC::Int),
          )
  ...
  fun tryst_notifier_set_timer(time_ptr : LibTcl::Time*) : Nil
  end

  # serviceModeHookProc: a no-op for the same reason as set_timer above.
  fun tryst_notifier_service_mode_hook(mode : LibC::Int) : Nil
  end

  fun tryst_notifier_init : Void*
    state = Tryst::Notifier.create_state_for(Thread.current)
    Box.box(state)
  end

  fun tryst_notifier_finalize(client_data : Void*) : Nil
  ...
  fun tryst_notifier_create_file_handler(fd : LibC::Int, mask : LibC::Int,
                                         proc : Void*, client_data : Void*) : Nil
    state = Tryst::Notifier.state_for(Thread.current)
    return unless state
  ...
  # is the same convention Tryst::EventSource's callbacks already use.
  fun tryst_notifier_file_handler_event_proc(ev_ptr : Void*, flags : LibC::Int) : LibC::Int
    return 0 if (flags & LibTcl::TCL_FILE_EVENTS) == 0

    event = ev_ptr.as(LibTcl::FileHandlerEvent*)
    if state = Tryst::Notifier.state_for(Thread.current)
  ...
  def tryst_notifier_time_span(time_ptr : LibTcl::Time*) : Time::Span?
    return if time_ptr.null?
    Time::Span.new(seconds: time_ptr.value.sec.to_i64, nanoseconds: time_ptr.value.usec.to_i64 * 1000)
  end

  fun tryst_notifier_wait_for_event(time_ptr : LibTcl::Time*) : LibC::Int
    state = Tryst::Notifier.state_for(Thread.current)
    return 0 unless state

    deadline = tryst_notifier_time_span(time_ptr)
    start = Time.instant

    loop do
      return 0 if Tryst::Notifier.poll_once(state)
