...
module Tryst
  ...
  class EventSource
    ...
    #
    # The trap is that `->tryst_event_source_setup` does not evaluate to
    ...
    SETUP_PROC = ->tryst_event_source_setup(Void*, LibC::Int)
    CHECK_PROC = ->tryst_event_source_check(Void*, LibC::Int)
    ...
    private def to_tcl_time(span : Time::Span) : LibTcl::Time
      ...
      # version-conditional type - see its declaration in interp.cr. Same
      # probe as interp.cr's two copies - see
      # src/tryst/tcltk_version_probe.cr for why it's a shared constant.
...
# Tryst::EventSource. Neither allocates and neither can raise.
#
...
fun tryst_event_source_setup(client_data : Void*, flags : LibC::Int)
  return unless event_source_serviced?(flags)
  state = client_data.as(Tryst::EventSource::State*)
...
fun tryst_event_source_check(client_data : Void*, flags : LibC::Int)
  return unless event_source_serviced?(flags)
  state = client_data.as(Tryst::EventSource::State*)
