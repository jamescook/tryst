...
# pixels - the Crystal counterpart to the throwaway C spike the ctk-yxa
# bake-off itself was decided on (see that bead's bd notes). Deliberately
# low-level (straight LibThorVG calls, no Surface wrapper yet - that's
# separate, not-yet-built scope) so THIS layer is proven before anything
