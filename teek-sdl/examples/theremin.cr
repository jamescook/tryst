# A theremin.
#
#   cd teek-sdl && crystal run examples/theremin.cr
#
...
# teek-sdl pulls in teek, but not the UI DSL - that is a separate entry
...
stream = Teek::SDL::AudioStream.new(
  Teek::SDL::AudioSpec.new(format: Teek::SDL::AudioFormat::S16LE, channels: 1, freq: SAMPLE_RATE)
)
...
session = Teek::UI.app(title: "theremin") do |builder|
