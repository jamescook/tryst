...
# the exit code is what spec/teek/sdl/image_text_spec.cr checks.
...
app = Teek::App.new(title: "image/text fixture")
app.show
app.update

viewport = Teek::SDL::Viewport.new(app, width: 160, height: 120, name: "vp_image_text")
renderer = viewport.renderer

blue = Teek::SDL::Color.new(0, 0, 255)
white = Teek::SDL::Color.new(255, 255, 255)

# --- Images ------------------------------------------------------------

png_dir = File.join(Dir.tempdir, "teek-sdl-image-fixture-#{Process.pid}")
Dir.mkdir_p(png_dir)
...
begin
  ...
  begin
    ...
    renderer.clear(blue)
    renderer.copy(image, dest: Teek::SDL::Rect.new(0, 0, image.width * 4, image.height * 4))
  ...
  via_from_file = Teek::SDL::Texture.from_file(renderer, png_path)
  begin
...
begin
  ...
  begin
    ...
    renderer.clear(blue)
    renderer.copy(blank, dest: Teek::SDL::Rect.new(0, 0, blank.width, blank.height))
    renderer.read_pixels do |pixels|
  ...
  begin
    renderer.clear(blue)
    renderer.copy(glyph, dest: Teek::SDL::Rect.new(0, 0, glyph.width, glyph.height))
    renderer.read_pixels do |pixels|
