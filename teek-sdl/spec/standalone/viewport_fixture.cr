...
green = Teek::SDL::Color.new(0, 255, 0)
...
begin
  ...
  renderer.clear(red)
  renderer.copy(placed, dest: Teek::SDL::Rect.new(0, 0, viewport.width // 2, viewport.height))
  renderer.read_pixels do |pixels|
    left = pixels[(pixels.width * 0.25).to_i, (pixels.height * 0.5).to_i]
    right = pixels[(pixels.width * 0.75).to_i, (pixels.height * 0.5).to_i]
...
streaming = renderer.create_texture(8, 8, Teek::SDL::Texture::Access::Streaming)
begin
  rows_seen = 0
...
target = renderer.create_texture(16, 16, Teek::SDL::Texture::Access::Target)
begin
...
spare.blend_mode = Teek::SDL::BlendMode::Blend
