...
describe Teek::SDL::Mixer do
  describe ".buffered" do
    it "produces exactly the format it was asked for" do
      spec = Teek::SDL::AudioSpec.new(format: Teek::SDL::AudioFormat::S16LE,
  ...
  describe "#generate" do
    it "rejects a buffer that is not a whole number of sample frames" do
      spec = Teek::SDL::AudioSpec.new(format: Teek::SDL::AudioFormat::S16LE, channels: 2)
      with_mixer(spec) do |mixer|
  ...
  describe ".decoders" do
    it "lists the formats this build can read, WAV among them" do
      with_mixer do
        ...
        Teek::SDL::Mixer.decoders.map(&.upcase).should contain("WAV")
      end
    end
  end
  ...
  describe ".default" do
    it "is what constructors fall back to, and can be replaced" do
      ...
      begin
        Teek::SDL::Mixer.default = mixer
        Teek::SDL::Mixer.default.should be(mixer)

        path = WavFixture.square(WavFixture.path("default.wav"), ms: 10)
        sound = Teek::SDL::Sound.new(path)
        begin
  ...
  describe "#destroy" do
    it "is idempotent and makes further use an error" do
      ...
      expect_raises(Teek::SDL::Error, /destroyed/) { mixer.gain }
    end
  end
end
