...
describe Teek::SDL::Gamepad do
  describe ".apply_dead_zone" do
    it "zeroes a value within the threshold" do
      Teek::SDL::Gamepad.apply_dead_zone(500).should eq(0)
      Teek::SDL::Gamepad.apply_dead_zone(-500).should eq(0)
    end

    it "passes a value at or beyond the threshold through unchanged" do
      Teek::SDL::Gamepad.apply_dead_zone(8_000).should eq(8_000)
      Teek::SDL::Gamepad.apply_dead_zone(-9_000).should eq(-9_000)
    end

    it "takes an explicit threshold" do
      Teek::SDL::Gamepad.apply_dead_zone(100, threshold: 50).should eq(100)
      Teek::SDL::Gamepad.apply_dead_zone(40, threshold: 50).should eq(0)
    end
  end
  ...
  describe ".attach_virtual" do
    it "creates a device .ids and .open can see, and refuses a second one" do
      ...
      begin
        Teek::SDL::Gamepad.ids.should contain(id)
        Teek::SDL::Gamepad.virtual_id.should eq(id)

        expect_raises(Teek::SDL::Error, /already attached/) do
          Teek::SDL::Gamepad.attach_virtual
        end
      ensure
  ...
  describe ".detach_virtual" do
    it "is idempotent, and clears .virtual_id" do
      ...
      Teek::SDL::Gamepad.detach_virtual
      Teek::SDL::Gamepad.detach_virtual
      Teek::SDL::Gamepad.virtual_id.should be_nil
    end
  end
  ...
  describe ".open" do
    it "opens the device by instance id" do
      with_virtual_gamepad do |gamepad|
        gamepad.name.should eq("Teek Virtual Gamepad")
        gamepad.attached?.should be_true
        gamepad.instance_id.should eq(Teek::SDL::Gamepad.virtual_id)
      end
    end

    it "raises for an unknown instance id" do
      expect_raises(Teek::SDL::Error, /SDL_OpenGamepad/) do
        Teek::SDL::Gamepad.open(999_999_u32)
      end
    end
  end
  ...
  describe "#button? / #axis" do
    it "starts at rest" do
      with_virtual_gamepad do |gamepad|
        Teek::SDL::Gamepad::BUTTONS.each { |button| gamepad.button?(button).should be_false }
        Teek::SDL::Gamepad::AXES.each { |axis| gamepad.axis(axis).should eq(0) }
      end
    end
  ...
  describe "#destroy" do
    it "is idempotent and makes further use an error" do
      id = Teek::SDL::Gamepad.attach_virtual
      begin
        gp = Teek::SDL::Gamepad.open(id)
        gp.destroy
        gp.destroy
        gp.destroyed?.should be_true
        expect_raises(Teek::SDL::Error, /closed/) { gp.name }
        expect_raises(Teek::SDL::Error, /closed/) { gp.set_virtual_button(:a, true) }
      ensure
        Teek::SDL::Gamepad.detach_virtual
      end
    end
  ...
  describe ".poll_events" do
    it "dispatches button, axis and device-added events to registered blocks" do
      seen = [] of String
      Teek::SDL::Gamepad.on_button { |instance_id, button, pressed| seen << "button #{instance_id} #{button} #{pressed}" }
      Teek::SDL::Gamepad.on_axis { |instance_id, axis, value| seen << "axis #{instance_id} #{axis} #{value}" }
      Teek::SDL::Gamepad.on_added { |instance_id| seen << "added #{instance_id}" }
      ...
      begin
        ...
        Teek::SDL::Gamepad.poll_events.should be > 0
        ...
        Teek::SDL::Gamepad.poll_events
      end
    end

    it "dispatches a removed event on detach" do
      seen = [] of String
      Teek::SDL::Gamepad.on_removed { |instance_id| seen << "removed #{instance_id}" }
  ...
  describe ".update_state" do
    it "does not raise when the subsystem was never brought up" do
      Teek::SDL::Gamepad.shutdown_subsystem
      Teek::SDL::Gamepad.update_state
    end
  end
end
