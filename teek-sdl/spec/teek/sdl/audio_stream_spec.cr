...
private def with_stream(spec = Teek::SDL::AudioSpec.new(format: Teek::SDL::AudioFormat::S16LE,
                          channels: 1, freq: 44_100), &)
  stream = Teek::SDL::AudioStream.new(spec)
  begin
    yield stream
  ensure
    stream.destroy
...
describe Teek::SDL::AudioStream do
  it "runs on a driver that makes no sound, so the suite stays quiet" do
    ...
    Teek::SDL::AudioStream.driver_name.should eq(ENV["SDL_AUDIO_DRIVER"])
  end

  it "finds a playback device" do
    Teek::SDL::AudioStream.device_count.should be >= 1
    Teek::SDL::AudioStream.available?.should be_true
  end

  it "keeps the format it was opened with" do
    spec = Teek::SDL::AudioSpec.new(format: Teek::SDL::AudioFormat::S16LE, channels: 1, freq: 22_050)
    with_stream(spec) do |stream|
      ...
      stream.format.should eq(Teek::SDL::AudioFormat::S16LE)
  ...
  describe "#queue" do
    ...
    it "counts frames by channel count, not by sample count" do
      stereo = Teek::SDL::AudioSpec.new(format: Teek::SDL::AudioFormat::S16LE, channels: 2)
  ...
  it "refuses to be used once destroyed" do
    ...
    expect_raises(Teek::SDL::Error, /destroyed/) { stream.queued_bytes }
  end
end
