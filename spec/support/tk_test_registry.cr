...
module TkTest
  REGISTRY = {} of String => Proc(Tryst::App, Nil)
end

# Registers a named Tk test case. The block runs against a shared,
# already-initialized Tryst::App (Tk_Init can only happen once per
# process - see spec/support/tk_worker.cr) - never construct a
# Tryst::App of your own inside a tk_test block. App#interp exposes the
...
def tk_test(name : String, *, darwin_only : Bool = false, &block : Tryst::App -> Nil) : Nil
  TkTest::REGISTRY[name] = block

  {% unless flag?(:tk_worker_mode) %}
    if darwin_only && !Tryst.platform.darwin?
      pending name
    else
      it name do
