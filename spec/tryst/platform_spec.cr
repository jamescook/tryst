require "../spec_helper"
require "../../src/tryst/platform"

# Pure-logic tests for Tryst::Platform - no Tk interpreter needed
# (precedent: ruby-tryst's test/test_platform.rb, which this mirrors -
# standalone platform detection with zero dependencies on the rest of tryst).
describe Tryst::Platform do
  ...
  it "detects darwin from an injected platform string" do
    p = Tryst::Platform.new("arm64-darwin24")
  ...
  it "detects linux from an injected platform string" do
    p = Tryst::Platform.new("x86_64-linux")
  ...
  it "detects windows (mingw) from an injected platform string" do
    p = Tryst::Platform.new("x64-mingw-ucrt")
    p.darwin?.should be_false
    p.linux?.should be_false
  ...
  it "detects windows (mswin) from an injected platform string" do
    p = Tryst::Platform.new("x64-mswin64_140")
    p.darwin?.should be_false
    p.linux?.should be_false
    p.windows?.should be_true
  end

  it "Tryst.platform returns the same instance every time" do
    Tryst.platform.should be(Tryst.platform)
  end
end
