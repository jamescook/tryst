...
private def with_scroll_defaults(auto : Bool? = nil, canvas : Bool? = nil, &)
  was_auto = Tryst::UI.auto_scroll
  was_canvas = Tryst::UI.auto_scroll_canvas
  # begin AFTER the reads, so the ensure below can't see them unassigned
  # (which would make them Bool? and fail to assign back).
  begin
    Tryst::UI.auto_scroll = auto unless auto.nil?
    Tryst::UI.auto_scroll_canvas = canvas unless canvas.nil?
    yield
  ensure
    Tryst::UI.auto_scroll = was_auto
    Tryst::UI.auto_scroll_canvas = was_canvas
  end
end

private def realize(session, default_scroll : Bool? = nil)
  app = FakeApp.new
  Tryst::UI::Realizer.new(app, session.document, default_scroll: default_scroll).realize
  app
end
...
describe "a natively scrollable widget" do
  it "is wrapped in a frame with the real widget one level deeper" do
    ...
    app.calls[0].args.should eq([".log"] of Tryst::TclArgValue)
    app.calls[1].args.should eq([".log.widget"] of Tryst::TclArgValue)
    app.calls[2].args.should eq([".log.vsb"] of Tryst::TclArgValue)
  end
  ...
  it "gives the widget the options, and the wrapper none of them" do
    ...
    app.calls[1].kwargs.should eq({"height" => 4} of String => Tryst::TclArgValue)
  end
  ...
  it "grids the widget and its scrollbar, weighting the widget's cell" do
    ...
    grids.map(&.args).should contain([".log.vsb"] of Tryst::TclArgValue)
    grids.map(&.args).should contain([".log.widget"] of Tryst::TclArgValue)
    grids.map(&.args).should contain([:columnconfigure, ".log", 0] of Tryst::TclArgValue)
    grids.map(&.args).should contain([:rowconfigure, ".log", 0] of Tryst::TclArgValue)
  end
  ...
  it "is vertical only unless x: asks for both" do
    ...
    vertical_only.calls.map(&.args).should_not contain([".log.hsb"] of Tryst::TclArgValue)

    both = realize(WidgetDslHarness.new.tap(&.list(:log, x: true)))
    both.calls.map(&.args).should contain([".log.hsb"] of Tryst::TclArgValue)
  end
  ...
  it "scroll: false leaves a plain widget with no wrapper at all" do
    ...
    app.calls.first.args.should eq([".log"] of Tryst::TclArgValue)
  ...
  it "wraps a text_area the same way, with the text widget inside" do
    ...
    app.calls[1].args.should eq([".notes.widget"] of Tryst::TclArgValue)
    app.calls[1].kwargs.should eq({"height" => 4} of String => Tryst::TclArgValue)
  ...
  it "wraps a tree and a table the same way, options landing on the treeview" do
    ...
    treeviews.map(&.args).should eq([
      [".hierarchy.widget"] of Tryst::TclArgValue,
      [".rows.widget"] of Tryst::TclArgValue,
    ])
    treeviews[0].kwargs.should be_empty
    treeviews[1].kwargs.should eq({
      "show"    => :headings,
      "columns" => ["name", "size"] of Tryst::TclArgValue,
    } of String => Tryst::TclArgValue)

    wrappers = app.calls.select { |call| call.cmd == "ttk::frame" }
    wrappers.map(&.args).should eq([
      [".hierarchy"] of Tryst::TclArgValue,
      [".rows"] of Tryst::TclArgValue,
    ])
  ...
  it "children of a scrollable widget are created inside the widget, not the wrapper" do
    ...
    app.calls.find { |call| call.cmd == "ttk::button" }
      .should_not(be_nil).args.should eq([".board.widget.go"] of Tryst::TclArgValue)
  end
