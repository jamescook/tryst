...
# Headless tests for Tryst::UI::TextContent - the exact Tk commands each
...
#
# Mirrors ruby-tryst's tryst-ui/test/test_text_content.rb, which is entirely
...
private def text_content(state : String = "normal") : {FakeApp, Tryst::UI::TextContent}
  app = FakeApp.new
  app.command_result = state
  {app, Tryst::UI::TextContent.new(app, ".notes")}
end
...
describe Tryst::UI::TextContent do
  describe "content" do
    ...
    it "insert applies any trailing format names to what it inserted" do
      ...
      sent(app).should eq([[:insert, "end", "boom", :error, "loud"] of Tryst::TclArgValue])
    end
    ...
    it "get reads a range, the whole buffer by default" do
      ...
      sent(app).should eq([
        [:get, "1.0", "end"] of Tryst::TclArgValue,
        [:get, "2.0", "end"] of Tryst::TclArgValue,
      ])
    end
    ...
    it "delete takes a range, or one character when given no end" do
      ...
      sent(app).should eq([
        [:delete, "1.0", "2.0"] of Tryst::TclArgValue,
        [:delete, "1.0"] of Tryst::TclArgValue,
      ])
    end
    ...
    it "replace swaps a range for new text in one call" do
      ...
      sent(app).should eq([[:replace, "1.0", "1.5", "howdy"] of Tryst::TclArgValue])
    end
    ...
    it "value asks for the buffer without Tk's synthetic trailing newline" do
      ...
      sent(app).should eq([[:get, "1.0", "end-1c"] of Tryst::TclArgValue])
    end
    ...
    it "value= empties the buffer before inserting the replacement" do
      ...
      sent(app).should eq([
        [:delete, "1.0", "end"] of Tryst::TclArgValue,
        [:insert, "1.0", "fresh"] of Tryst::TclArgValue,
      ])
    end
    ...
    it "clear deletes everything" do
      ...
      sent(app).should eq([[:delete, "1.0", "end"] of Tryst::TclArgValue])
    end
  end
  ...
  describe "indices" do
    it ":end and :cursor stand in for Tk's own end and insert" do
      ...
      sent(app).map(&.[](1)).should eq(["end", "insert"] of Tryst::TclArgValue)
    end
    ...
    it "any other index is passed through verbatim" do
      ...
      sent(app).map(&.[](1)).should eq(
        ["1.0", "end-1c", "insert +1 line", "sel.first", "@12,34", "my_marker"] of Tryst::TclArgValue)
    end
  end
  ...
  describe "mutating a read-only widget" do
    it "lifts the state for the call and restores it after" do
      ...
      states.should eq([
        {"state" => "normal"} of String => Tryst::TclArgValue,
        {"state" => "disabled"} of String => Tryst::TclArgValue,
      ])
    end
  ...
  describe "formats" do
    it "format defines display properties under a name" do
      ...
      call.args.should eq([:tag, :configure, :error] of Tryst::TclArgValue)
      call.kwargs.should eq({"foreground" => "red", "underline" => true} of String => Tryst::TclArgValue)
    end
    ...
    it "apply_format applies one to a range, clear_format takes it off again" do
      ...
      sent(app).should eq([
        [:tag, :add, :error, "1.0", "1.5"] of Tryst::TclArgValue,
        [:tag, :remove, :error, "1.0", "end"] of Tryst::TclArgValue,
      ])
    end
    ...
    it "delete_format removes the definition itself" do
      ...
      sent(app).should eq([[:tag, :delete, :error] of Tryst::TclArgValue])
    end
    ...
    it "on_format_click binds a left click on the format, callback and all" do
      ...
      call = app.calls.last
      call.args.first(4).should eq([:tag, :bind, :link, "<Button-1>"] of Tryst::TclArgValue)
      call.args[4].should be_a(Proc(Array(String), Tryst::CallbackSignal, Nil))
    end
    ...
    it "on_format takes any event pattern, with or without the angle brackets" do
      ...
      app.calls.map { |call| call.args[3]? }.should eq(["<Double-Button-1>", "<Enter>"] of Tryst::TclArgValue)
    end

    # `tag bind` rather than a raw tcl_eval is what puts the callback under
    # tryst's own leak-safe reconcile - see TagBindInterceptor.
    it "routes format bindings through the widget's own tag bind" do
      app, text = text_content
      text.on_format_click(:link) { }

      app.calls.last.cmd.should eq(".notes")
      app.calls.last.args.first(2).should eq([:tag, :bind] of Tryst::TclArgValue)
    end
  end
  ...
  describe "markers" do
    it "add_marker places one, remove_marker unsets it" do
      ...
      sent(app).should eq([
        [:mark, :set, :here, "1.0"] of Tryst::TclArgValue,
        [:mark, :unset, :here] of Tryst::TclArgValue,
      ])
    end
    ...
    it "mark_gravity reads with no direction and sets with one" do
      ...
      sent(app).should eq([
        [:mark, :gravity, :here] of Tryst::TclArgValue,
        [:mark, :gravity, :here, :left] of Tryst::TclArgValue,
      ])
    end
  end
  ...
  describe "search" do
    it "searches forward from the cursor by default" do
      ...
      sent(app).should eq([[:search, "--", "needle", "insert", "end"] of Tryst::TclArgValue])
    end
    ...
    it "forwards each switch, before the -- that ends them" do
      ...
      sent(app).should eq([
        [:search, "-backward", "-regexp", "-nocase", "--", "-dash", "1.0", "end"] of Tryst::TclArgValue,
      ])
    end
  end
  ...
  describe "view, cursor and state" do
    it "scroll_to scrolls an index into view" do
      ...
      sent(app).should eq([[:see, "50.0"] of Tryst::TclArgValue])
    end
    ...
    it "index canonicalises an index expression" do
      ...
      sent(app).should eq([[:index, "insert +1 line"] of Tryst::TclArgValue])
    end
    ...
    it "cursor reads the insert mark, cursor= moves it" do
      ...
      sent(app).should eq([
        [:index, "insert"] of Tryst::TclArgValue,
        [:mark, :set, "insert", "1.0"] of Tryst::TclArgValue,
      ])
    end
    ...
    it "read_only reports whether Tk's own -state is disabled" do
      ...
      disabled_app.calls.last.args.should eq([:cget, "-state"] of Tryst::TclArgValue)
  ...
  describe "embedded images" do
    it "insert_image embeds one in the text flow" do
      app, text = text_content
      image = Tryst::UI::Image.new("tryst_ui_image_1", ".notes")

      text.insert_image("1.0", image: image)

      call = app.calls.find { |call| call.args.first? == :image }.should_not be_nil
      call.args.should eq([:image, :create, "1.0"] of Tryst::TclArgValue)
      call.kwargs.should eq({"image" => "tryst_ui_image_1"} of String => Tryst::TclArgValue)
    end

    # A raw Tryst::Photo works too - it needs a live App to exist at all,
    ...
    it "takes a plain Tcl image name as well" do
      ...
      app.calls.find { |call| call.args.first? == :image }
        .should_not(be_nil).kwargs.should eq({"image" => "some_tcl_image"} of String => Tryst::TclArgValue)
    end
...
describe "Handle#text_content" do
  it "hands back a TextContent for a realized text_area" do
    ...
    handle.text_content.should be_a(Tryst::UI::TextContent)
  end
  ...
  it "raises before realize, like every other live accessor" do
    ...
    expect_raises(Tryst::UI::NotRealizedError) { handle.text_content }
  end
