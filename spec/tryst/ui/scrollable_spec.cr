...
#
# Mirrors ruby-tryst's tryst-ui/test/test_scrollable.rb.
...
private def add_label_after_realize(app : FakeApp, y : Bool = true) : Nil
  ...
  parent = session.document.find(:scroller).as(Tryst::UI::Node)
  added = session.document.create(type: :label, name: :added,
    opts: {:text => "later"} of Symbol => Tryst::TclArgValue)
  parent.add_child(added)
  Tryst::UI::Realizer.new(app, session.document).realize_subtree(added, parent)
end
...
describe "ui.scrollable" do
  ...
  it "creates children inside the embedded viewport, not under its own path" do
    ...
    labels.should eq([
      [".scroller.canvas.viewport.first"] of Tryst::TclArgValue,
      [".scroller.canvas.viewport.second"] of Tryst::TclArgValue,
    ])
  end
  ...
  it "builds a frame holding a canvas holding a viewport frame" do
    ...
    frames.should eq([
      [".scroller"] of Tryst::TclArgValue,
      [".scroller.canvas.viewport"] of Tryst::TclArgValue,
    ])

    canvas = app.calls.find { |call| call.cmd == "canvas" }.should_not be_nil
    canvas.args.should eq([".scroller.canvas"] of Tryst::TclArgValue)
    # A focus ring around the scrolling region would just be visual noise.
    canvas.kwargs.should eq({"highlightthickness" => 0} of String => Tryst::TclArgValue)
  end
  ...
  it "embeds the viewport in the canvas as a window item at the origin" do
    ...
    create.args.should eq([:create, :window, 0, 0] of Tryst::TclArgValue)
    create.kwargs.should eq(
      {"window" => ".scroller.canvas.viewport", "anchor" => "nw"} of String => Tryst::TclArgValue)
  end
  ...
  it "packs the viewport's children to fill it" do
    ...
    pack.should_not(be_nil).kwargs.should eq(
      {"fill" => "both", "expand" => true} of String => Tryst::TclArgValue)
  end
  ...
  describe "scrollbars" do
    it "attaches a vertical scrollbar by default, driving the canvas" do
      ...
      bars.map(&.args).should eq([[".scroller.vsb"] of Tryst::TclArgValue])
      bars.first.kwargs.should eq(
        {"orient" => "vertical", "command" => ".scroller.canvas yview"} of String => Tryst::TclArgValue)
    end
    ...
    it "attaches both scrollbars when x: is on too" do
      ...
      app.calls.select { |call| call.cmd == "ttk::scrollbar" }.map(&.args).should eq([
        [".scroller.vsb"] of Tryst::TclArgValue,
        [".scroller.hsb"] of Tryst::TclArgValue,
      ])
    end
  ...
  describe "content tracking" do
    it "re-measures the scrollregion whenever the viewport resizes" do
      ...
      bind.block.call([] of String, Tryst::CallbackSignal.new)

      configure = app.calls.last
      configure.cmd.should eq(".scroller.canvas")
      configure.args.should eq([:configure] of Tryst::TclArgValue)
      configure.kwargs.should eq({"scrollregion" => "0 0 100 400"} of String => Tryst::TclArgValue)
    end
    ...
    it "holds the content at the canvas's width when not scrolling horizontally" do
      ...
      bind.block.call(["640"], Tryst::CallbackSignal.new)
      app.calls.last.args.first.should eq(:itemconfigure)
      app.calls.last.kwargs.should eq({"width" => "640"} of String => Tryst::TclArgValue)
    end
  ...
  describe "wheel scrolling" do
    ...
    it "gives the canvas, the viewport and every descendant the same bindtag" do
      ...
      ] of Tryst::TclArgValue)

      tagged.each do |call|
        call.args[1].as(Array(Tryst::TclArgValue)).last.should eq("TrystScrollRegion_scroller_canvas")
      end
    end
    ...
    it "appends the tag rather than replacing what Tk already set" do
      ...
      set.args[1].should eq([
        ".scroller.canvas", "Canvas", ".", "all", "TrystScrollRegion_scroller_canvas",
      ] of Tryst::TclArgValue)
    end
    ...
    it "binds the wheel and X11's button pair on the tag, not on a widget" do
      ...
      wheel = app.binds.select { |b| b.widget == "TrystScrollRegion_scroller_canvas" }
    ...
    it "binds the shifted pair as well when x: is on" do
      ...
      app.binds.select { |b| b.widget == "TrystScrollRegion_scroller_canvas" }.map(&.event).should eq([
    ...
    it "binds nothing at all when neither axis scrolls" do
      ...
      app.binds.map(&.widget).should_not contain("TrystScrollRegion_scroller_canvas")
    ...
    it "scrolls the canvas by whole units, scaled like Tk's own scrollbar" do
      ...
      wheel.block.call(["120"], Tryst::CallbackSignal.new)
      app.calls.last.args.should eq(["yview", :scroll, -3, :units] of Tryst::TclArgValue)
    ...
    it "rounds a half notch away from zero rather than down to nothing" do
      ...
      wheel.block.call(["20"], Tryst::CallbackSignal.new)
      app.calls.last.args.should eq(["yview", :scroll, -1, :units] of Tryst::TclArgValue)
    end
    ...
    it "scrolls sideways on the shifted wheel" do
      ...
      shifted.block.call(["120"], Tryst::CallbackSignal.new)
      app.calls.last.args.should eq(["xview", :scroll, -3, :units] of Tryst::TclArgValue)

      up = app.binds.find { |b| b.event == "<Shift-Button-4>" }.should_not be_nil
      up.block.call([] of String, Tryst::CallbackSignal.new)
      app.calls.last.args.should eq(["xview", :scroll, -1, :units] of Tryst::TclArgValue)
    end
  end
  ...
  # headlessly (it always constructs a real Tryst::App at realize), so
  ...
  describe "adding content after realize" do
    it "creates the new child in the viewport, not under the scrollable's own path" do
      ...
      created.should eq([".scroller.canvas.viewport.added"] of Tryst::TclArgValue)
    end
    ...
    it "puts the new child on the scroll region's bindtag, so the wheel works over it" do
      ...
      tagged.args[1].as(Array(Tryst::TclArgValue)).last.should eq("TrystScrollRegion_scroller_canvas")
    end
    ...
    it "tags every descendant of the new subtree, not just its root" do
      ...
      leaf = session.document.create(type: :label, name: :leaf,
        opts: {:text => "deep"} of Symbol => Tryst::TclArgValue)
      branch.add_child(leaf)
      parent.add_child(branch)
      Tryst::UI::Realizer.new(app, session.document).realize_subtree(branch, parent)
    ...
    it "leaves an ordinary container's own path as where its children go" do
      ...
      parent = session.document.find(:wrapper).as(Tryst::UI::Node)
