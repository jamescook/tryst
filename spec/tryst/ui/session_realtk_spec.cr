...
# Tryst::UI::Session#realize/#run/#run_async always construct a brand-new
# Tryst::App (a real Tcl/Tk interpreter), so - like Interp#mainloop (see
# spec/tryst/mainloop_spec.cr) - these need a genuinely fresh subprocess
# per test rather than the shared tk_worker every other Tk-touching spec
# uses. Headless build-phase coverage (Tryst::UI.app never touching Tk at
# all) lives in spec/tryst/ui/session_spec.cr instead.
describe "Tryst::UI::Session#realize" do
...
describe "Tryst::UI::Session's realize-only surface" do
