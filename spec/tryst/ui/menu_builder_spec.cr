...
# ruby-tryst's tryst-ui/test/test_menu_dsl.rb to what MenuBuilder itself
...
describe Tryst::UI::MenuBuilder do
  it "menu declares a nested cascade node with a label" do
    document = Tryst::UI::Document.new
    builder = Tryst::UI::MenuBuilder.new(document, [document.root])
  ...
  it "menu nests recursively for submenus" do
    document = Tryst::UI::Document.new
    stack = [document.root]
    builder = Tryst::UI::MenuBuilder.new(document, stack)
  ...
  it "item appends a command entry, its block stashed as opts[:command]" do
    ...
    item_node.opts[:command].as(Proc(Array(String), Tryst::CallbackSignal, Nil)).call([] of String, Tryst::CallbackSignal.new)
  ...
  it "checkbox appends a checkbutton entry, bind: translated into the variable option" do
    document = Tryst::UI::Document.new
    builder = Tryst::UI::MenuBuilder.new(document, [document.root])
    wrap = Tryst::UI::Var.new("::tryst_ui_var_1", true)
    ...
    node.opts[:variable].should eq("::tryst_ui_var_1")
  end
  ...
  it "radio appends a radiobutton entry bound to a var with a value" do
    document = Tryst::UI::Document.new
    builder = Tryst::UI::MenuBuilder.new(document, [document.root])
    size = Tryst::UI::Var.new("::tryst_ui_var_1", "small")
