...
# Headless tests for Tryst::UI::Realizer, built against FakeApp
...
# it runs identically against a real Tryst::App or FakeApp). A couple of
...
Tryst::UI::WidgetTypes.register(
  Tryst::UI::WidgetType.new(type: :__test_realize_gauge__, tk_command: "ttk::progressbar")
)

describe Tryst::UI::Realizer do
  ...
  it "realizes a type registered from outside this library" do
    ...
    create.args.should eq([".cpu"] of Tryst::TclArgValue)
    create.kwargs.should eq({"maximum" => 100} of String => Tryst::TclArgValue)
  ...
  it "realizing a nested tree creates every widget at a hierarchical path" do
    ...
    app.calls[0].args.should eq([".controls"] of Tryst::TclArgValue)
    app.calls[1].args.should eq([".controls.go"] of Tryst::TclArgValue)
    app.calls[1].kwargs.should eq({"text" => "Go"} of String => Tryst::TclArgValue)
  end
  ...
  it "realizing a group creates a labelframe with its children inside" do
    ...
    app.calls[0].args.should eq([".settings"] of Tryst::TclArgValue)
    app.calls[0].kwargs.should eq({"text" => "Settings"} of String => Tryst::TclArgValue)
    app.calls[1].args.should eq([".settings.verbose"] of Tryst::TclArgValue)
  end
  ...
  it "reserved DSL-only opts never leak through to a widget-creation call" do
    session = WidgetDslHarness.new
    node = session.document.create(type: :button, opts: {:text => "Go", :scroll => false} of Symbol => Tryst::TclArgValue)
    session.document.root.add_child(node)

    app = FakeApp.new
    Tryst::UI::Realizer.new(app, session.document).realize

    app.calls.first.kwargs.should eq({"text" => "Go"} of String => Tryst::TclArgValue)
  end
  ...
  it "arranges a container's own children before descending into their own subtrees" do
    ...
    pack_paths.should eq([".outer", ".outer.inner", ".outer.inner.deep"] of Tryst::TclArgValue)
  end
  ...
  it "raw_op runs its block, with the live app, during link" do
    ...
    node.raw_block = Proc(Tryst::UI::AppContract, Nil).new { |app| received_app = app }
    session.document.root.add_child(node)
  ...
  it "on_close: wires App#on_close to the node's own realized path" do
    session = WidgetDslHarness.new
    closed = false
    block = Proc(Array(String), Tryst::CallbackSignal, Nil).new { |_v, _s| closed = true }
    session.button(:go)
  ...
  it "wires an event binding on the node's own path when target: is nil" do
    ...
    btn.events << Tryst::UI::EventBinding.new(
      event: "<Button-1>",
      handler: Proc(Array(String), Tryst::CallbackSignal, Nil).new { |_v, _s| fired = true }
    )
    ...
    app.binds.first.block.call([] of String, Tryst::CallbackSignal.new)
  ...
  it "an event binding targeting a widget declared later resolves once the whole tree is realized" do
    ...
    trigger.events << Tryst::UI::EventBinding.new(
      event: "<Button-1>",
      handler: Proc(Array(String), Tryst::CallbackSignal, Nil).new { |_v, _s| fired = true },
      target: :downstream
    )
  ...
  it "raises if an event binding's target isn't found in the document" do
    ...
    expect_raises(ArgumentError, /nope/) { Tryst::UI::Realizer.new(FakeApp.new, session.document).realize }
  end

  it "raises for a node type with no registered WidgetType" do
    session = WidgetDslHarness.new
    session.document.root.add_child(Tryst::UI::Node.new(type: :__not_a_real_widget_type__))

    expect_raises(ArgumentError, /__not_a_real_widget_type__/) { Tryst::UI::Realizer.new(FakeApp.new, session.document).realize }
  end

  it "a type registered arranged: false is skipped by the plain pack arrangement" do
    Tryst::UI::WidgetTypes.register(Tryst::UI::WidgetType.new(type: :__test_realizer_unarranged__, tk_command: "toplevel", arranged: false))

    session = WidgetDslHarness.new
    session.document.root.add_child(Tryst::UI::Node.new(type: :__test_realizer_unarranged__, name: :w))
  ...
  it "realize_subtree creates and arranges a new node under an already-realized parent" do
    ...
    app = FakeApp.new
    realizer = Tryst::UI::Realizer.new(app, session.document)
    realizer.realize

    parent_node = session.document.root.children.first
    new_node = session.document.create(type: :button, name: :added, opts: {:text => "Added"} of Symbol => Tryst::TclArgValue)
    ...
    app.calls.last.args.should eq([".controls.added"] of Tryst::TclArgValue)
  end

  it "realize_subtree raises if the parent node isn't realized yet" do
    session = WidgetDslHarness.new
    parent = Tryst::UI::Node.new(type: :panel)
    child = Tryst::UI::Node.new(type: :button)

    expect_raises(ArgumentError, /must already be realized/) do
      Tryst::UI::Realizer.new(FakeApp.new, session.document).realize_subtree(child, parent)
    end
  end
  ...
  it "column flow layout packs the first/last child with pad, others with gap, plus align: :stretch's fill/cross-fill" do
    ...
    pack_a.kwargs.should eq({"side" => "top", "pady" => [4, 0] of Tryst::TclArgValue, "padx" => 4, "fill" => "x"} of String => Tryst::TclArgValue)
    pack_b.kwargs.should eq({"side" => "top", "pady" => [8, 4] of Tryst::TclArgValue, "padx" => 4, "fill" => "both", "expand" => true} of String => Tryst::TclArgValue)
  end
  ...
  it "row flow layout uses its own side/main_pad/cross_pad/fill axis, the opposite of column's" do
    ...
    pack_a.kwargs.should eq({"side" => "left", "padx" => [0, 0] of Tryst::TclArgValue, "pady" => 0, "anchor" => "n"} of String => Tryst::TclArgValue)
  end
  ...
  it "grid layout places each cell's widget with its own row/column/sticky/padding" do
    ...
    grid_calls.map(&.args.first).should eq([".g.name_label", ".g.name_field"] of Tryst::TclArgValue)
    grid_calls[0].kwargs.should eq({"row" => 0, "column" => 0, "sticky" => "ew", "padx" => 4, "pady" => 4} of String => Tryst::TclArgValue)
    grid_calls[1].kwargs.should eq({"row" => 0, "column" => 1, "sticky" => "ew", "padx" => 4, "pady" => 4} of String => Tryst::TclArgValue)
  end
  ...
  it "stretch: configures columnconfigure/rowconfigure on the grid's own path with weight 1" do
    ...
    col_config.first.args.should eq([:columnconfigure, ".g", 0] of Tryst::TclArgValue)
    col_config.first.kwargs.should eq({"weight" => 1} of String => Tryst::TclArgValue)

    row_config = app.calls.select { |call| call.cmd == "grid" && call.args.first == :rowconfigure }
    row_config.size.should eq(1)
    row_config.first.args.should eq([:rowconfigure, ".g", 1] of Tryst::TclArgValue)
  end
  ...
  it "an overlay-tagged child is placed at its anchor's relx/rely/anchor, -in its canvas parent" do
    ...
    place_call.args.should eq([".board.status"] of Tryst::TclArgValue)
    place_call.kwargs.should eq({"in" => ".board", "relx" => 1.0, "rely" => 1.0, "anchor" => "se"} of String => Tryst::TclArgValue)
  end
  ...
  it "an overlay-tagged child is never packed alongside its plain-arranged siblings" do
    ...
    app.calls.select { |call| call.cmd == "place" }.map(&.args.first).should eq([".board.status"] of Tryst::TclArgValue)
  end
  ...
  it "two overlays on the same canvas each land at their own anchor" do
    ...
    place_calls.map(&.args.first).should eq([".board.status", ".board.pause"] of Tryst::TclArgValue)
    place_calls[0].kwargs.should eq({"in" => ".board", "relx" => 0.0, "rely" => 0.0, "anchor" => "nw"} of String => Tryst::TclArgValue)
    place_calls[1].kwargs.should eq({"in" => ".board", "relx" => 1.0, "rely" => 0.0, "anchor" => "ne"} of String => Tryst::TclArgValue)
  end
  ...
  it "a menu_bar creates a real menu widget and attaches it to the root window's own -menu" do
    ...
    menu_create.args.should eq([".mb"] of Tryst::TclArgValue)
    menu_create.kwargs.should eq({"tearoff" => 0} of String => Tryst::TclArgValue)

    attach = app.calls.find! { |call| call.cmd == "." && call.args.first == :configure }
    attach.kwargs.should eq({"menu" => ".mb"} of String => Tryst::TclArgValue)
  end
  ...
  it "a nested .menu realizes as its own menu widget, added as a cascade entry on its parent" do
    ...
    menu_creates.map(&.args.first).should eq([".mb", ".mb.file"] of Tryst::TclArgValue)

    cascade = app.calls.find! { |call| call.cmd == ".mb" && call.args == [:add, :cascade] }
    cascade.kwargs.should eq({"label" => "File", "menu" => ".mb.file"} of String => Tryst::TclArgValue)
  end
  ...
  it "item adds a command entry carrying its own label and command callback" do
    ...
    handler = entry.kwargs["command"].as(Proc(Array(String), Tryst::CallbackSignal, Nil))
  ...
  it "separator adds a childless separator entry" do
    ...
    entry.kwargs.should eq({} of String => Tryst::TclArgValue)
  end
