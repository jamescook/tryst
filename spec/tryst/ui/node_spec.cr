require "../../spec_helper"
require "../../../src/tryst/ui/node"
require "../../../src/tryst/ui/document"
require "../../../src/tryst/ui/image"
require "../../../src/tryst/ui/var"

# Pure-logic tests for Tryst::UI::Node - no Tk interpreter needed. Mirrors
# ruby-tryst's tryst-ui/test/test_node.rb.
#
# Two ruby-tryst cases aren't ported as-is:
...
describe Tryst::UI::Node do
  it "has the expected defaults" do
    node = Tryst::UI::Node.new(type: :button)

    node.type.should eq(:button)
    node.name.should be_nil
    node.key.should be_nil
    node.opts.should eq({} of Symbol => Tryst::TclArgValue)
    node.children.should eq([] of Tryst::UI::Node)
    node.events.should eq([] of Tryst::UI::EventBinding)
    node.images.should eq([] of Tryst::UI::Image)
    node.vars.should eq([] of Tryst::UI::Var)
    node.grow?.should be_false
    node.realized.should be_nil
    node.scope.should be(Tryst::UI::Scope::TOP_LEVEL)
  end

  it "scope is settable and readable" do
    scope = Tryst::UI::Scope.new(:sidebar)
    node = Tryst::UI::Node.new(type: :button, scope: scope)
  ...
  it "an explicit key overrides the name-derived default" do
    node = Tryst::UI::Node.new(type: :button, name: :save, key: "custom")

    node.key.should eq("custom")
  end

  it "opts are retained verbatim" do
    opts = {:text => "Hi", :width => 10} of Symbol => Tryst::TclArgValue
    node = Tryst::UI::Node.new(type: :label, opts: opts)
  ...
  it "grow and realized are settable after construction" do
    ...
    node.grow = true
    node.realized = Tryst::UI::RealizedNode.new(app: nil, path: ".fake")

    node.grow?.should be_true
    node.realized.should eq(Tryst::UI::RealizedNode.new(app: nil, path: ".fake"))
  end

  it "each visits self then children depth-first pre-order" do
    root = Tryst::UI::Node.new(type: :column)
    a = root.add_child(Tryst::UI::Node.new(type: :button, name: :a))
    b = root.add_child(Tryst::UI::Node.new(type: :column, name: :b))
    c = b.add_child(Tryst::UI::Node.new(type: :button, name: :c))
  ...
  it "to_a collects every node as an Array" do
    root = Tryst::UI::Node.new(type: :column)
    root.add_child(Tryst::UI::Node.new(type: :button, name: :a))
  ...
  it "find returns the first node the block accepts" do
    root = Tryst::UI::Node.new(type: :column)
    a = root.add_child(Tryst::UI::Node.new(type: :button, name: :a))
    root.add_child(Tryst::UI::Node.new(type: :button, name: :b))
  ...
  it "find stops walking as soon as it matches" do
    root = Tryst::UI::Node.new(type: :column)
    branch = root.add_child(Tryst::UI::Node.new(type: :column, name: :branch))
    branch.add_child(Tryst::UI::Node.new(type: :button, name: :wanted))
    root.add_child(Tryst::UI::Node.new(type: :button, name: :never_reached))
  ...
  it "document defaults to nil" do
    Tryst::UI::Node.new(type: :button).document.should be_nil
  end

  it "document is settable at construction" do
    document = Tryst::UI::Document.new
    node = Tryst::UI::Node.new(type: :button, document: document)
  ...
  it "remove_child removes it from the parent's children" do
    parent = Tryst::UI::Node.new(type: :column)
    a = parent.add_child(Tryst::UI::Node.new(type: :button, name: :a))
    b = parent.add_child(Tryst::UI::Node.new(type: :button, name: :b))
  ...
  it "remove_child clears the removed node's own parent" do
    parent = Tryst::UI::Node.new(type: :column)
    child = parent.add_child(Tryst::UI::Node.new(type: :button))
  ...
  it "parent is nil before being attached" do
    Tryst::UI::Node.new(type: :button).parent.should be_nil
  end

  it "root's logical_path is a bare dot" do
    root = Tryst::UI::Node.new(type: :root)
  ...
  it "logical_path for a top-level named node" do
    root = Tryst::UI::Node.new(type: :root)
    child = root.add_child(Tryst::UI::Node.new(type: :button, name: :save))
  ...
  it "logical_path nests through named ancestors" do
    root = Tryst::UI::Node.new(type: :root)
    column = root.add_child(Tryst::UI::Node.new(type: :column, name: :toolbar))
    button = column.add_child(Tryst::UI::Node.new(type: :button, name: :save))
  ...
  it "logical_path uses the auto-generated key when unnamed" do
    root = Tryst::UI::Node.new(type: :root)
    child = root.add_child(Tryst::UI::Node.new(type: :button, key: "#anon1"))
  ...
  it "display_name is the bare type when unnamed" do
    Tryst::UI::Node.new(type: :column).display_name.should eq("column")
  end

  it "display_name includes the name when given" do
    Tryst::UI::Node.new(type: :column, name: :ctrl).display_name.should eq("column(:ctrl)")
  end
end
