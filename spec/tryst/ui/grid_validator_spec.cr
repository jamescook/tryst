...
require "../../../src/tryst/ui/grid_validator"
# See handle_spec.cr's own note on requiring the real Realizer.
require "../../../src/tryst/ui/realizer"

# Pure-logic tests for Tryst::UI::GridValidator - no Tk interpreter
# needed. Mirrors the grid-specific cases of ruby-tryst's
# tryst-ui/test/test_validator.rb; the other (non-grid) cases in that
# file are covered by validator_spec.cr instead.
describe Tryst::UI::GridValidator do
  it "a grid child missing a cell is reported" do
    ...
    errors = [] of String
    Tryst::UI::GridValidator.call(grid, document.root, document, errors)
  ...
  it "two widgets placed in the same cell are reported as a collision" do
    ...
    errors = [] of String
    Tryst::UI::GridValidator.call(grid_node, session.document.root, session.document, errors)
  ...
  it "check_stray_cell reports a cell position whose parent isn't a grid" do
    ...
    errors = [] of String
    Tryst::UI::GridValidator.check_stray_cell(stray, panel, errors)
  ...
  it "check_stray_cell says nothing when the parent IS a grid" do
    ...
    placed.cell_position = Tryst::UI::CellPosition.new(row: 0, col: 0)
    grid.add_child(placed)

    errors = [] of String
    Tryst::UI::GridValidator.check_stray_cell(placed, grid, errors)
  ...
  it "check_stray_cell says nothing for a node with no cell position at all" do
    ...
    errors = [] of String
    Tryst::UI::GridValidator.check_stray_cell(node, nil, errors)
