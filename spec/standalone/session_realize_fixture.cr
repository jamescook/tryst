...
# Standalone verification for Teek::UI::Session#realize/#run_async -
# needs its own subprocess (see spec/teek/ui/session_realtk_spec.cr)
# because Session#realize always constructs a brand-new Teek::App (a
...
session = Teek::UI.app(title: "session realize fixture") do |builder|
  builder.panel(:controls, &.button(:go, text: "Go"))
end

app = session.realize
raise "expected #realize to return a Teek::App" unless app.is_a?(Teek::App)
