require "../../spec_helper"
require "../../../src/teek/ui/mouse_events"

# Pure-logic tests for Teek::UI::MouseEvents - no Tk interpreter needed.
#
...
describe Teek::UI::MouseEvents do
  ...
  it "spells a right click as Button-3 alone off macOS" do
    Teek::UI::MouseEvents.right_click_events(false).should eq(["<Button-3>"])
  end
  ...
  it "adds macOS's secondary-click gestures on darwin, without losing Button-3" do
    events = Teek::UI::MouseEvents.right_click_events(true)
  ...
  it "resolves RIGHT_CLICK_EVENTS from the running platform" do
    expected = Teek::UI::MouseEvents.right_click_events(Teek.platform.darwin?)

    Teek::UI::MouseEvents::RIGHT_CLICK_EVENTS.should eq(expected)
  end
  ...
  it "gives every platform's patterns as bracketed Tk event sequences" do
    [true, false].each do |darwin|
      Teek::UI::MouseEvents.right_click_events(darwin).each do |event|
  ...
  it "accepts exactly the two menu handle types on_right_click can pop up" do
    Teek::UI::MouseEvents::MENU_HANDLE_TYPES.should eq([:menu, :context_menu])
  end
end
