...
# not production code - every other headless teek-ui spec (Realizer,
# WidgetDSL, ...) will exercise them constantly just by using them, the
# same way ruby-teek's own fake_app.rb has no dedicated behavior tests of
# its own. This file just confirms the recorder itself actually records,
# since nothing else in the suite consumes it yet.
#
# The signature-drift contract check itself (ruby-teek's
...
# real Teek::App/Teek::Window: if either drifts from what FakeApp/
...
describe FakeApp do
  it "command logs the command, positional args, and keyword args" do
    ...
    call.args.should eq([".btn"] of Teek::TclArgValue)
    call.kwargs.should eq({"side" => "left", "padx" => 10} of String => Teek::TclArgValue)
  end
  ...
  it "bind logs the widget, event, substitutions, and block" do
    ...
    call.block.call([] of String, Teek::CallbackSignal.new)
