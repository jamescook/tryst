...
#
# Mirrors ruby-teek's teek-ui/test/test_split.rb, which is entirely
...
describe "the :split and :pane widget types" do
  ...
  describe "orientation:" do
    ...
    it "reaches Tk as an ordinary widget option" do
      ...
      app.calls.find { |call| call.cmd == "ttk::panedwindow" }
        .should_not(be_nil).kwargs.should eq({"orient" => "vertical"} of String => Teek::TclArgValue)
    end
  end
  ...
  describe "weight:" do
    ...
    it "is left off the add call entirely when unset" do
      ...
      adds.map(&.kwargs).should eq([{} of String => Teek::TclArgValue])
    end
    ...
    it "sends a zero weight rather than treating it as unset" do
      ...
      app.calls.find { |call| call.cmd == ".panes" && call.args.first? == :add }
        .should_not(be_nil).kwargs.should eq({"weight" => 0} of String => Teek::TclArgValue)
    end
  end
  ...
  it "realizes a panedwindow with each pane as a frame beneath it" do
    ...
    app.calls.find { |call| call.cmd == "ttk::panedwindow" }
      .should_not(be_nil).args.should eq([".panes"] of Teek::TclArgValue)
    frames = app.calls.select { |call| call.cmd == "ttk::frame" }.map(&.args)
    frames.should eq([[".panes.left"] of Teek::TclArgValue, [".panes.right"] of Teek::TclArgValue])
  end
  ...
  it "adds each pane to its panedwindow with its weight, in declaration order" do
    ...
    adds.map(&.args).should eq([
      [:add, ".panes.left"] of Teek::TclArgValue,
      [:add, ".panes.right"] of Teek::TclArgValue,
    ])
    adds.map(&.kwargs).should eq([
      {"weight" => 1} of String => Teek::TclArgValue,
      {"weight" => 3} of String => Teek::TclArgValue,
    ])
  end
  ...
  it "the validator rejects a pane whose parent isn't a split" do
    ...
    session.document.find(:wrapper).as(Teek::UI::Node).add_child(stray)

    error = expect_raises(Teek::UI::ValidationError) do
