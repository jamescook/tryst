require "../spec_helper"
require "../../src/teek/native_window"
...
describe Teek::NativeWindow do
  describe "#pointer" do
    it "hands back an address for the platforms whose handle is one" do
      cocoa = Teek::NativeWindow.new(path: ".f", kind: Teek::NativeWindowKind::Cocoa, value: 0x1234_u64)
      cocoa.pointer.address.should eq(0x1234)

      win32 = Teek::NativeWindow.new(path: ".f", kind: Teek::NativeWindowKind::Win32, value: 0x5678_u64)
    ...
    it "refuses on X11, where the handle is a number and not an address" do
      ...
      x11 = Teek::NativeWindow.new(path: ".f", kind: Teek::NativeWindowKind::X11, value: 0x2a00003_u64)
  ...
  describe "#covers_toplevel?" do
    it "is true only for Cocoa, where a widget has no window of its own" do
      path = ".f"
      Teek::NativeWindow.new(path: path, kind: Teek::NativeWindowKind::Cocoa, value: 1_u64)
        .covers_toplevel?.should be_true
      Teek::NativeWindow.new(path: path, kind: Teek::NativeWindowKind::X11, value: 1_u64)
        .covers_toplevel?.should be_false
      Teek::NativeWindow.new(path: path, kind: Teek::NativeWindowKind::Win32, value: 1_u64)
        .covers_toplevel?.should be_false
    end
  end

  it "prints the kind, the path and the handle in hex" do
    handle = Teek::NativeWindow.new(path: ".viewport", kind: Teek::NativeWindowKind::X11, value: 255_u64)
